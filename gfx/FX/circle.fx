...
struct VS_OUTPUT 
{
   ...
   float2 TexCoord :        TEXCOORD0;
   float vStrength :		TEXCOORD1; // we alll float down here
};
...
float4 Circle_PS( VS_OUTPUT In ) : COLOR
{
	...
	//0     0.5   1.0
	//c1 -> c2 -> c3	
	float3 StrengthColor = lerp( float3(1.0f,0.0f,0.0f), float3(1.0f,1.0f,0.0f), saturate(In.vStrength * 2.0f) );
	StrengthColor = lerp( StrengthColor, float3(0.0f,1.0f,0.0f), saturate((In.vStrength - 0.5f)*2.0f) );
	
	return float4(DiffuseColor * StrengthColor, DiffuseColor * 0.3f);
}
